...
# 이슈3: 쓰레기 값(Garbage) 목록 — 단독으로 들어오면 의미 없는 값
GARBAGE_DETAIL = {'년생', '년', '생', '년생)', '(년생)'}
...
for idx, r in df.iterrows():
    ...
    # 날짜 (^^xsd:date 타입 지정 → 기간 범위 FILTER 쿼리 가능)
    if safe(r.get('HappenDate')):
        t.append(f'  ex:happenDate "{r["HappenDate"]}"^^xsd:date ;')
    if safe(r.get('NoticeSDate')):
        t.append(f'  ex:noticeSDate "{r["NoticeSDate"]}"^^xsd:date ;')
    if safe(r.get('NoticeEDate')):
        t.append(f'  ex:noticeEDate "{r["NoticeEDate"]}"^^xsd:date ;')
    ...
    if safe(r.get('CareRegNo')):
        ...
        # careNm은 AnimalShelter 노드가 보유 → 중복 제거
