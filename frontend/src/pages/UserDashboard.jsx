...
export default function UserDashboard() {
  ...
  return <main className="mypage"><div className="my-shell">
    ...
    <section className="my-plus-section" aria-labelledby="plus-preview-title"><header><div><p>ANIMAL-ROUTE PLUS · 데모</p><h2 id="plus-preview-title">기록을 더 오래, 더 편하게 관리해요</h2><span>핵심 증상 확인과 응급 안내는 언제나 무료입니다.</span></div><button type="button" aria-haspopup="dialog" aria-expanded={plusOpen} onClick={openPlus}>Plus 전체 보기 →</button></header><div className="my-plus-grid"><article><span className="my-plus-badge">Plus</span><h3>우리 아이 건강 기록</h3><p>최근 건강 경로 3건은 무료로 보고, 전체 기록과 월별 변화는 Plus에서 모아볼 수 있어요.</p><button type="button" onClick={openPlus}>기록 관리 알아보기 →</button></article><article><span className="my-plus-badge">Plus</span><h3>놓치기 쉬운 건강 일정</h3><p>예방접종·검진·복약 일정을 등록하고 필요한 시점에 알림을 받아보세요.</p><button type="button" onClick={openPlus}>일정 관리 알아보기 →</button></article><article><span className="my-plus-badge">Plus</span><h3>저장한 병원과 시설</h3><p>자주 찾는 장소, 방문 전 체크리스트, 증상 요약을 한곳에 저장해요.</p><button type="button" onClick={openPlus}>저장 기능 알아보기 →</button></article></div><small className="my-plus-demo-note">본선 데모 화면입니다. 결제·구독·실제 알림 및 기록 저장은 아직 연결되지 않았습니다.</small></section>
