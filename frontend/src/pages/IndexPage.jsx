...
const PORTS = [
  ...
  { port:5180, label:"Vite Dev Server", desc:"프론트엔드 개발서버",   status:"running",   layer:"L4 프론트엔드", color:C.L4 },
  ...
  { port:3031, label:"Apache Jena Fuseki", desc:"SPARQL 엔드포인트 (RDF)", status:"running", layer:"L3 KG",      color:"#0d9488" },
  { port:5280, label:"KOAH ReSpec (nginx)", desc:"온톨로지 명세 + Turtle 협상", status:"running", layer:"L5 문서", color:"#dc2626" },
];

const DEV_ENV = [
  { label:"Frontend",  stack:"React 18 + Vite + Tailwind CSS",       color:C.L4, port:5180 },
  ...
  { label:"RDF Store", stack:"Apache Jena Fuseki (TDB2)",            color:"#0d9488", port:3031 },
  { label:"Ontology",  stack:"nginx + ReSpec / def_animal.ttl (KOAH)", color:"#dc2626", port:5280 },
];
...
const ARCH_LAYERS = [
  ...
  {
    num:"L3", title:"데이터베이스 레이어", subtitle:"PostgreSQL + PostGIS · :5435 / Fuseki · :3031",
    color:C.L3, port:5435,
    desc:"정제된 데이터를 영구 저장합니다. PostgreSQL/PostGIS 가 관계형·공간 데이터를, Apache Jena Fuseki(TDB2) 가 RDF 트리플과 SPARQL 추론을 담당합니다.",
    chips:[
      "PostgreSQL (PostGIS 확장)", "공간 인덱스 (GiST)",
      "Apache Jena Fuseki (TDB2)", "SPARQL 1.1 엔드포인트 (:3031)",
      "named graphs (ds01..ds16)", "매핑 테이블 (15개 데이터셋)",
      "메타데이터 관리 (출처·기준시점)", "인증 & 세션 스토어",
    ],
  },
  {
    num:"L4", title:"프론트엔드 레이어", subtitle:"React + Vite + Tailwind · :5180",
    color:C.L4, port:5180,
  ...
  {
    num:"L5", title:"온톨로지 / 문서 레이어",
    subtitle:"KOAH ReSpec · nginx · :5280  ·  https://knowledgemap.kr/koah/def/",
    color:"#dc2626", port:5280,
    desc:"Korea Open Animal Hub(KOAH) 어휘를 W3C ReSpec 으로 발행하고, Accept 헤더 기반 content negotiation 으로 HTML / Turtle / RDF/XML 응답을 분기합니다. UK Public Sector URI 101 (4-layer: def/id/doc/set) 정책을 따르며, 모든 인스턴스 URI 는 이 네임스페이스로 dereference 됩니다.",
    chips:[
      "nginx 1.27 (alpine)", "W3C ReSpec",
      "OWL Turtle (def_animal.ttl)", "Content negotiation (HTML ↔ Turtle)",
      "UK URI 101 4-layer", "KOAH 네임스페이스",
      "R2RML-style 데이터셋 매핑 (15개)", "ds01..ds16 dereference",
      "legacy 301 redirect (/def/animal → /koah/def)",
    ],
  },
];
...
const FLOW_STEPS = [
  ...
  },
  {
    ...
    items:[
      "RDF Triple 생성 (Subject–Predicate–Object) — backend/kg/builders/dsXX_*.py",
      "URI 정책: https://knowledgemap.kr/koah/{def|id|doc|set}/... (UK URI 101)",
      "KOAH 어휘 매핑 (def_animal.ttl, terms.yaml 단일 진실원천)",
      "KISTI 증상분류 ↔ 검역본부 질병정보 조인 (ds13/15/16)",
      "결정적 빌더 — sha1 검증 가능",
    ],
  },
  {
    num:"04", title:"적재 (Load to KG)", layer:"L3", color:"#0d9488", port:3031,
    items:[
      "Apache Jena Fuseki(TDB2) 에 ds01..ds16 named graph 로 적재",
      "ops/fuseki/load.sh — 16개 turtle 파일 일괄 import",
      "SPARQL 1.1 엔드포인트 노출 (:3031/knowledgemap/query)",
      "owl:sameAs / 추론 체인(증상→질병→진료과→병원) 활성화",
    ],
  },
  {
    num:"05", title:"서비스 (Serve)", layer:"L2 API", color:C.L2, port:8000,
    items:[
      "FastAPI REST 엔드포인트 제공 (:8000)",
      "PostGIS 공간 쿼리 — 반경 N km 시설 검색",
      "Fuseki 위임 SPARQL 쿼리 (증상 → 질병 → 병원 추천)",
      "구조동물 RFID ↔ 분실동물 크로스 매칭",
    ],
  },
  {
    num:"06", title:"표현 (Present)", layer:"L4", color:C.L4, port:5180,
    items:[
      "Canvas Force-directed 지식그래프 (:5180)",
      "WGS84 마커 기반 반려동물 지도",
  ...
  {
    num:"07", title:"발행 (Publish Ontology)", layer:"L5", color:"#dc2626", port:5280,
    items:[
      "KOAH 어휘 W3C ReSpec 발행 (:5280/koah/def/)",
      "Accept 협상: HTML / Turtle / RDF/XML 자동 분기",
      "OWL 온톨로지 (def_animal.ttl) — terms.yaml 단일 진실원천",
      "URI dereference 보장 (UK URI 101 cool URIs)",
      "/def/animal/* → /koah/def/* 301 legacy redirect",
    ],
  },
];

const RDF_TRIPLES = [
  {s:"id:dog/12345",            p:"def:hasSymptom",       o:"id:symptom/구토",                src:"KISTI 증상분류 (ds13)"},
  {s:"id:symptom/구토",         p:"def:indicatesDisease", o:"id:disease/개파보",              src:"동물질병 마스터 (ds15)"},
  {s:"id:disease/개파보",       p:"def:treatedAt",        o:"id:animal-hospital/366000001",   src:"동물병원 (ds01)"},
  {s:"id:animal-hospital/366…", p:"def:locatedIn",        o:"id:region/서울/강남구",          src:"행정구역"},
  {s:"id:animal-shelter/456",   p:"def:protectedBy⁻¹",    o:"id:abandoned/3149999/2025-128",  src:"구조동물 (ds08)"},
  {s:"id:lost/777",             p:"def:matchingCandidate",o:"id:abandoned/3149999/2025-128",  src:"분실↔구조 매칭"},
  {s:"def:Disease",             p:"rdfs:isDefinedBy",     o:"<https://knowledgemap.kr/koah/def/>", src:"KOAH ontology"},
];
      ...
      <div style={{ border:`1px solid ${C.border}`, borderRadius:10, overflow:"hidden",
        ...
        <div style={{ padding:"8px 12px", background:"#f8fafc",
          ...
          <div style={{ flex:1, margin:"0 10px", background:C.white,
            ...
            border:`1px solid ${C.border}` }}>
            localhost:5180{SCREENS.find(s=>s.id===active)?.path}
          </div>
          <code style={{ fontSize:9, color:C.green, background:"#f0fdf4",
            padding:"2px 7px", borderRadius:4 }}>:5180</code>
        </div>
        <Preview />
      </div>
