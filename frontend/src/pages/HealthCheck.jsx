...
export default function HealthCheck() {
  ...
  return <div className="hc-page">
    ...
    <main className="hc-shell">
      ...
      {step === 4 && result && <section className="hc-content hc-enter hc-route-page">
        <header className="hc-route-hero"><div><p className="hc-eyebrow">STEP 4 · HEALTH ROUTE</p><h1>입력한 증상에서<br />다음 확인 정보까지 연결했어요.</h1><p>{result.medical_disclaimer}</p></div><div className="hc-route-illustration" aria-hidden="true"><img src={getPetAvatar(draft.pet.species)} alt="" /><span>✚</span></div></header>
        <section className="hc-route-summary hc-route-summary--visual" aria-label="건강 경로 요약"><article><i aria-hidden="true">⌂</i><div><small>보호자가 입력한 증상</small><strong>{draft.symptom}</strong></div></article><span aria-hidden="true">→</span><article><i aria-hidden="true">✓</i><div><small>내가 확인한 표준 증상</small><strong>{selected?.label}</strong></div></article><span aria-hidden="true">→</span><article><i aria-hidden="true">⌖</i><div><small>주요 활동 지역</small><strong>{draft.pet.region}</strong></div></article></section>
        <div className="hc-route-columns">
          <section className="hc-route-card hc-route-diseases"><header><h2>관련 가능성 높은 질병</h2><small>진단·발생 확률이 아닙니다</small></header><div>{result.diseases.slice(0, 3).map((disease, index) => <details key={disease.uri || disease.name} open={index === 0}><summary><b>{index + 1}</b><div><strong>{disease.name}</strong><span>{index === 0 ? '연결 근거 있음' : '연결 정보 확인'}</span></div><i aria-hidden="true">›</i></summary><p>{disease.department}</p><small>{disease.source} · 수의사 진단 필요</small></details>)}</div><button type="button" className="hc-route-more" onClick={() => setStep(3)}>질병 정보 더 보기</button></section>
          <section className="hc-route-card hc-route-evidence"><header><h2>연결된 근거 정보</h2><small>데이터상 연결 정보</small></header><ul><li><i aria-hidden="true">⌂</i><span>기관지천 정보</span><button type="button">자세히 보기 ›</button></li><li><i aria-hidden="true">⌘</i><span>기관 학적 정보</span><button type="button">자세히 보기 ›</button></li><li><i aria-hidden="true">♧</i><span>만성 기관지염 정보</span><button type="button">자세히 보기 ›</button></li></ul><p className="hc-route-evidence-note">본 정보는 일반적인 참고 자료이며, 반드시 수의사의 진단이 필요합니다.</p></section>
          <section className="hc-route-card hc-route-hospitals"><header><h2>추천 병원</h2><small>{draft.pet.region} 기준 · 전화 확인</small></header><div>{result.hospitals.slice(0, 3).map((hospital, index) => <article key={hospital.facility_id || hospital.name}><i aria-hidden="true">✚</i><div><strong>{hospital.name}</strong><span>{index === 0 ? '전화로 진료 가능 여부를 먼저 확인해 주세요.' : '현재 진료 가능 여부는 전화 확인이 필요해요.'}</span><p><a href={`tel:${hospital.phone}`}>전화하기</a><button type="button">지도 보기</button></p></div></article>)}</div></section>
        </div>
        <footer className="hc-route-safety"><SafetyNotice urgent>응급 상황이 아닌 일반적인 호흡·의식 위험 답변은 일반 결과보다 먼저 차단됩니다.</SafetyNotice><button type="button" className="hc-primary" onClick={reset}>새 건강 경로 시작하기 →</button></footer>
      </section>}
    </main>
  </div>
}
